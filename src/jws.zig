...
/// Decoded JWS header and payload.
...
pub const Codec = struct {
    ...
    /// Signature algorithm used to sign and verify the encoded/decoded JWS signatures.
    sig_algorithm: ?signature.Algorithm = null,

    /// Returns a JWS containing the given payload.
    ///
    /// The signature part of the JWS is created using the Codec's configured algorithm
    /// and the given key.
    ///
    /// The caller owns the memory allocated for the JWS.
    ...
    /// Decodes and verifies a JWS and returns the payload part as a value of type T.
    ///
    ...
    pub fn decode(self: Codec, comptime PayloadT: type, allocator: Allocator, s: []const u8) !Decoded(PayloadT) {
        ...
        const jws_arena_alloc = try allocator.create(ArenaAllocator);
        jws_arena_alloc.* = ArenaAllocator.init(allocator);
        const jws_arena = jws_arena_alloc.allocator();
        ...
        const header = try json.parseFromSliceLeaky(Header, jws_arena, header_json, .{ .allocate = .alloc_always });
        ...
        const payload = try json.parseFromSliceLeaky(PayloadT, jws_arena, payload_json, .{ .allocate = .alloc_always });

        return Decoded(PayloadT){
            .arena = jws_arena_alloc,
            .header = header,
            .payload = payload,
        };
    }
...
/// Used internally to parse the JWS string parts.
...
test "JWS encode with alg=hs256" {
...
test "JWS decode alg=hs256" {
    const codec = Codec{ .sig_algorithm = test_hs256.algorithm() };
    const enc_str = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0ZXN0X2ZpZWxkIjo1fQ.kwVT7OeKoswn-5rjGKuKr7NUGQx5rAuRA3THFtwqp3Y";

    const dec_jws = try codec.decode(struct { test_field: i32 }, std.testing.allocator, enc_str);
    defer dec_jws.deinit();

    try std.testing.expectEqual(5, dec_jws.payload.test_field);
}

test "JWS decode alg=none" {
    const codec = Codec{};
